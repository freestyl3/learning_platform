...
class ModuleCreateView(LoginRequiredMixin, UserPassesTestMixin, CreateView):
    ...
    def test_func(self):
        # author = get_object_or_404(Course, pk=self.kwargs.get('course_id')).author
...
class ModuleDetailView(LoginRequiredMixin, UserPassesTestMixin, DetailView):
    ...
    def test_func(self):
        # module = get_object_or_404(Module, pk=self.kwargs.get('lesson_id'))
    ...
    # def get_object(self):
    #     return get_object_or_404(Module, pk=self.kwargs.get('lesson_id'))
...
class HiddenModuleListView(LoginRequiredMixin, UserPassesTestMixin, ListView):
    ...
    def test_func(self):
        author = Course.objects.get(pk=self.kwargs.get(self.pk_url_kwarg)).author
        # author = get_object_or_404(Course, pk=self.kwargs.get(self.pk_url_kwarg)).author
        return self.request.user == author

    def get_queryset(self):
        return Module.objects.filter(course_id=self.kwargs.get(self.pk_url_kwarg), hidden=True)


@login_required
def toggle_module(request, module_id):
    # lesson = get_object_or_404(Module, pk=lesson_id)
